...
*   **Architectural Impact:** This determines the operational workflow, user experience friction, and compliance/audit capabilities.
    ...
    *   **Path B (File-Based Approval):** Write approval requests to filesystem (e.g., `.codepipe/approvals/pending/<id>.json`) and poll for corresponding approval files. Allows asynchronous approvals and script-based automation, but requires documentation of approval protocol.
