...
import { RedactionEngine } from '../../../telemetry/logger';
...
export default class ContextSummarize extends TelemetryCommand {
  ...
  async run(): Promise<void> {
    ...
    let repoConfig: ReturnType<typeof requireConfig>;
    try {
    ...
    await this.runWithTelemetry(
      {
        ...
        spanAttributes: {
          patterns: typedFlags.path?.length ?? 0,
          force: typedFlags.force,
        },
      },
      async (ctx) => {
        ctx.logger?.info('Context summarization started', {
