 ...
 * - Section 3.10.4: `ai-feature pr create` command flow
 */
...
export default class PRCreate extends Command {
        ...
        await withSpan(
          ...
          async (span) => {
            ...
            if (!isCodeApproved(manifest)) {
              ...
              this.error(
                'Code approval gate is required before creating PR. Run "ai-feature approve code --signer <email>" first.',
            ...
            if (!validationsPassed) {
              ...
              this.error(
                'Validations (lint/test/build) must pass before creating PR. Run "ai-feature validate" first.',
                { exit: PRExitCode.HUMAN_ACTION_REQUIRED }
              );
            }
  ...
  private generatePRBody(manifest: { title?: string; source?: string }): string {
    ...
    if (manifest.title) {
      ...
      lines.push('Generated by AI Feature Pipeline');
    }
    lines.push('');
    ...
    lines.push('---');
    lines.push('');
    lines.push('🤖 Generated with [AI Feature Pipeline](https://github.com/codemachine/pipeline)');
