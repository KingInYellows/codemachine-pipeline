...
export interface TelemetryResult {
  ...
  // eslint-disable-next-line @typescript-eslint/no-restricted-types -- logging metadata is intentionally open-ended
...
export abstract class TelemetryCommand extends Command {
  ...
  protected surfaceExitCode(error: unknown): number {
    if (error instanceof CliError) {
      return error.code === CliErrorCode.RUN_DIR_NOT_FOUND ? 10 : error.exitCode;
    }

    if (error && typeof error === 'object') {
      const maybeOclif = (error as { oclif?: { exit?: unknown } }).oclif;
      if (maybeOclif && typeof maybeOclif.exit === 'number') {
        return maybeOclif.exit;
      }
