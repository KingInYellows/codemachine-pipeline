...
The adapter uses a dual-strategy architecture. The engine tries each strategy in registration order; the first whose `canHandle()` returns `true` is selected.
...
│           ▼                                     ▼            │
│  ┌──────────────────────────────┐  ┌──────────────────────┐  │
│  │ CodeMachineCLIStrategy       │  │ CodeMachineStrategy  │  │
│  │ (preferred)                  │  │ (legacy fallback)    │  │
│  │  ┌────────────────────────┐  │  │  ┌────────────────┐  │  │
│  │  │ CodeMachineCLIAdapter  │  │  │  │ CodeMachine    │  │  │
│  │  │ ┌──────────────────┐   │  │  │  │ Runner         │  │  │
│  │  │ │ BinaryResolver   │   │  │  │  └────────────────┘  │  │
│  │  │ │ (env→npm→PATH)   │   │  │  └──────────────────────┘  │
│  │  │ └──────────────────┘   │  │                            │
...
| Strategy            | Name              | Binary Resolution          | Credential Handling |
| ------------------- | ----------------- | -------------------------- | ------------------- |
| CodeMachineCLIStrategy | `codemachine-cli` | 3-tier resolver (env, npm optionalDep, PATH) | stdin (secure) |
| CodeMachineStrategy | `codemachine`     | Direct config path         | Environment vars    |

The `codemachine-cli` strategy is registered first. If the binary is not found, it reports unavailable and the engine falls back to the legacy `codemachine` strategy automatically.
