...
echo "[example-openai-key]" | grep -E "sk-ant-[A-Za-z0-9_-]{48}"
...
for i in {1..100}; do
  echo "[example-openai-key]$i-with-extra-chars-here" >> test.txt
done
...
1. Developer commits file with real API key: `[example-openai-key]`
2. PR created
