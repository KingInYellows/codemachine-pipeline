...
- [ ] 2.1: Create `src/workflows/cycleIssueOrderer.ts` with `orderCycleIssues()`
  function
- [ ] 2.2: Implement Kahn's algorithm for topological sort using the
  `IssueRelationType.blocks` edges derived from both `relations` and
  `inverseRelations`
- [ ] 2.3: Within each topological level, sort by priority ascending
  (1=Urgent, 2=High, 3=Medium, 4=Low, 0=None at the end)
...
- [ ] 4.3: Implement command flow:
  ...
  2. Resolve cycle: use `--cycle` flag or `fetchActiveCycle(teamId)`,
     normalizing a cycle name to the matching `{id, name, number}` record
  3. Fetch issues: `fetchCycleIssues(resolvedCycle.id)`
  4. Order issues: `orderCycleIssues(issues)`
  5. Filter terminal-state issues
  6. If `--dry-run`: render ordered list and exit
  7. Create cycle run directory: `.codepipe/runs/cycle-<resolvedCycle.id>/`
