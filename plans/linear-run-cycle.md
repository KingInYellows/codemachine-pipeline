...
> Status: Historical plugin-repo plan kept for reference. It targets the
> companion `yellow-linear` plugin repository, not this repo, and is superseded
> here by [`plans/cycle-runner-core.md`](./cycle-runner-core.md).
...
4. **Priority + dependency ordering** — topological sort on `blocks` edges
   derived from `relations` and `inverseRelations`, falling back to priority
   order for unrelated issues.
...
- [ ] 4.2: In `--plan-only` mode, bypass `/linear:work`'s interactive routing.
  Reuse its non-interactive setup steps to validate the issue, load context,
  and write the brainstorm doc, then invoke `/workflows:plan` directly and
  stop before `/workflows:work`
